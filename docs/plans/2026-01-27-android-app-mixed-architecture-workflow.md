...
success_criteria:
  ...
  - FFmpeg/N_m3u8DL-RE integration for HLS streams
...
verify:
  type: shell
  command: cd backend && python -m pytest test_core.py -v
...
verify:
  type: artifact
  path: app/src/main/res/layout/item_recent_video.xml
  assert:
    kind: exists
...
verify:
  type: shell
  command: cd backend && python -m pytest test_platforms.py -v
...
verify:
  type: shell
  command: cd backend && python -c "import ffmpeg; print(ffmpeg.check_version())"

- [ ] **Step 41: N_m3u8DL-RE Integration Testing**
action: Test N_m3u8DL-RE compatibility with Android environment for protected streams
loop: false
max_iterations: 1
verify:
  type: shell
...
verify:
  type: shell
  command: adb shell am profile start com.videodl.android > cpu_profile.txt
